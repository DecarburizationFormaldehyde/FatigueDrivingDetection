...
def cnnlstm(data):
    ...
    while True:
        ...
        if i == 15:
            break
    plt.plot(range(len(train_scaled[timesteps:])), train_scaled[timesteps:])
    plt.plot(range(len(train_scaled[timesteps:]), len(train_scaled[timesteps:]) + len(
        scaler.inverse_transform(test_scaled[timesteps:].reshape(-1, 1)))),
             scaler.inverse_transform(test_scaled[timesteps:].reshape(-1, 1)))
    plt.show()
    return scaler.inverse_transform(test_scaled[timesteps:].reshape(-1, 1))

    # return min_max_normalization(result)
